...
rah  = ln(z₂/z₁) / (k·u*) = ln(0.2/0.1)/(k·u*)            max(1.0)
```
⚠️ **Ikki xil z₂ bor** (chalkashtirmaslik):
- `z2_rah = 0.2 m` — **rah** log hadi uchun (z1 = 0.1 m) → ln(2)
- `z2 = 2.0 m` — **stability (ψ)** uchun
...
### 5.3 Kaskad (`anchor_method='cascade'`)
`cimec → plan_a → plan_b → pysebal → default`
— avval land-cover zonasida, keyin butun ROI'da; hech biri chiqmasa `default` persentil.

**Qabul sharti:** `(LST_hot − LST_cold) ≥ 1.0 K` (`min_dt`)

### 5.4 point_anchor — kandidatlardan BITTA piksel
SEBAL_ID `process_tile` da **avtomatik** `anchor_mode='point_anchor'` ga o'tadi
(cold/hot dT va hot suv balansi **ayni bir pikselga** tayanishi uchun):
```
cold = eng SOVUQ piksel   → Reducer.min(2): 'min'=LST, 'min1'=RN_G0  (izchil juft)
hot  = eng ISSIQ piksel   → Reducer.max(2): 'max'=LST, 'max1'=RN_G0
...
2. **Tuproq** (shu nuqtada): OpenLandMap USDA tekstura klassi (1–12) →
   FAO-56 **Table 5.1** (θ_FC, θ_WP, REW):
   ```
   TEW = 1000 × (θ_FC − 0.5·θ_WP) × Ze        Ze = 0.10 m
   TEW = max(TEW, REW + 1)                     [TEW > REW kafolati]
   ```
   | Tuproq | θ_FC | θ_WP | REW (mm) |
   |---|---|---|---|
   | Sand | 0.12 | 0.045 | 4.5 |
   | Loam | 0.25 | 0.12 | 9.0 |
   | Silt loam | 0.29 | 0.15 | 9.5 |
   | Clay | 0.36 | 0.22 | 10.0 |
...
```
ETr24 = ASCE KUNLIK PM (alfalfa), MAHALLIY STANDART kalendar kun
        Cn = 1600, Cd = 0.38, G = 0
        ⚠️ overpass VAQTI emas (aks holda 24h oynasi siljiydi)
        ⚠️ UTC kun ham emas — MAHALLIY kun (`utc_offset`, App.5-A; DST YO'Q)
────────────────────────────────────────
...
```
TEW/REW — OpenLandMap teksturadan RASTER (Table 5.1), Ze = 0.10
har kun:
...
| kB⁻¹ | 2.3 | z₀h |
| z1 / z2_rah / z2 | 0.1 / 0.2 / 2.0 m | rah / stability |
| z_blending | 200 m | u₂₀₀ |
| ASCE alfalfa (soatlik) | Cn=66, Cd=0.25, G=0.04Rn | ETR_INST |
| ASCE alfalfa (kunlik) | Cn=1600, Cd=0.38, G=0 | ETr24 |
