...
def hot_pixel_etrf(image, roi, hot_mask, window_days=30, ze=0.10,
    ...
    # --- 2. Tuproq teksturasi SHU NUQTADA (OpenLandMap USDA klass 1-12) ---
    tex = (ee.Image(TEXTURE).select('b0').reduceRegion(
        ee.Reducer.first(), hot_pt, 250, maxPixels=1e9).get('b0')).getInfo()
    ...
    if verbose:
        tex_name = {1:'Clay',2:'SiCl',3:'SaCl',4:'ClLo',5:'SiClLo',6:'SaClLo',
                    7:'Lo',8:'SiLo',9:'SaLo',10:'Si',11:'LoSa',12:'Sa'}.get(tex_class,'?')
        print(f"    💧 Hot suv balansi @({lat:.3f},{lon:.3f}): tuproq={tex_name}"
              f"(#{tex_class}) TEW={TEW:.1f} REW={REW:.1f} | De={De:.1f} "
